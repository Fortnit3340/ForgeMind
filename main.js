...
const state = {
  ...
  ai: { engine:"lemonade", provider:"Lemonade", baseUrl:"http://127.0.0.1:13305/v1", model:"" }
};
...
async function listAIModels(){
  const base = String(state.ai.baseUrl || "http://127.0.0.1:13305/v1").trim().replace(/\/+$/,"");
  const response = await fetch(`${base}/models`);
  const text = await response.text();
  if (!response.ok) throw new Error(`AI model endpoint ${response.status}: ${text.slice(0,500)}`);
  let data;
  try { data = JSON.parse(text); }
  catch { throw new Error("AI model endpoint returned invalid JSON."); }
  return Array.isArray(data?.data) ? data.data : [];
}

async function chat(messages){
  const engine = state.ai.engine || "lemonade";
  const base = String(
    state.ai.baseUrl ||
    (engine === "lemonade" ? "http://127.0.0.1:13305/v1" : "")
  ).trim().replace(/\/+$/,"");

  if (!base) throw new Error("AI Base URL is not set. Open Settings.");

  let model = String(state.ai.model || "").trim();

  // Lemonade is local-first: no API key is required for the local runtime.
  if (engine === "lemonade" && !model) {
    const models = await listAIModels();
    model = String(models?.[0]?.id || models?.[0]?.name || "").trim();
    if (!model) {
      throw new Error("Lemonade is reachable, but no model is available. Download a model in Lemonade, then use Detect Models.");
    }
  }

  if (engine === "cloud") {
    const key = getApiKey();
    if (!key) throw new Error("No cloud API key saved. Open Settings and add your own API key.");
...
function createWindow(){
  ...
  win.webContents.on("did-fail-load", (_event, errorCode, errorDescription) => {
    send("startup-error", {ok:false, errorCode, errorDescription});
  });
}
...
ipcMain.handle("ai:chat",(_,messages)=>chat(messages));
